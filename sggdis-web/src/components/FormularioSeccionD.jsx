...
function FormularioSeccionD({ datos }) {
  ...
  useEffect(() => {
    async function cargarSeccion() {
      try {
        setCargando(true);
        setError(null);
        const respuesta = await fetch(`${API_BASE_URL}/api/guias-inspeccion/1/secciones/D`);
        if (!respuesta.ok) {
        ...
        setGrupos(agruparPorArticulo(datosApi.items));
              ...
              console.error('Error al cargar la Sección D:', err);
              setError('No se pudo cargar la Sección D. Verificá que el backend esté corriendo.');
       } finally {
        setCargando(false);
      }
    }
  ...
  return (
    <div className="pagina">
      ...
      <nav className="tabs">
        {TABS.map((tab, i) => (
          <span key={tab} className={`tabs__item ${i === 3 ? 'tabs__item--activo' : ''}`}>
            {tab}
          </span>
        ))}
      </nav>
      ...
      <main className="tarjeta">
        <div className="tarjeta__encabezado">
          <span className="tarjeta__etiqueta">SECCIÓN D</span>
          <div className="tarjeta__titulo-fila">
            <h2>Medidas de Saneamiento</h2>
...
export default FormularioSeccionD;