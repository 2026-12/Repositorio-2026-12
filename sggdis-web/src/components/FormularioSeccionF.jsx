...
import './FormularioSeccionF.css';

// Ajustar el puerto si el tuyo es distinto al que muestra Swagger
const API_BASE_URL = 'https://localhost:7119';

const TABS = [
  'Aspectos Generales', 'Cocina y Preparación', 'Bodega de Insumos', 'Medidas de Saneamiento',
  'Manejo de Desechos', 'Área de Consumo', 'Servicio a Domicilio', 'Cierre y Dictamen',
];
...
function FormularioSeccionF({ datos }) {
  ...
  useEffect(() => {
    async function cargarSeccion() {
      try {
        setCargando(true);
        setError(null);
        const respuesta = await fetch(`${API_BASE_URL}/api/guias-inspeccion/1/secciones/F`);
        if (!respuesta.ok) {
        ...
        setGrupos(agruparPorArticulo(datosApi.items));
              ...
              console.error('Error al cargar la Sección F:', err);
              setError('No se pudo cargar la Sección F. Verificá que el backend esté corriendo.');
       } finally {
        setCargando(false);
      }
    }
  ...
  return (
    <div className="pagina">
      ...
      <nav className="tabs">
        {TABS.map((tab, i) => (
          <span key={tab} className={`tabs__item ${i === 5 ? 'tabs__item--activo' : ''}`}>
            {tab}
          </span>
        ))}
      </nav>
      ...
      <main className="tarjeta">
        <div className="tarjeta__encabezado">
          <span className="tarjeta__etiqueta">SECCIÓN F</span>
          <div className="tarjeta__titulo-fila">
            <h2>Área de Consumo (Comedor)</h2>
...
export default FormularioSeccionF;