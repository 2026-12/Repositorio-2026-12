...
function FormularioSeccionE({ datos }) {
  ...
  useEffect(() => {
    async function cargarSeccion() {
      try {
        setCargando(true);
        setError(null);
        const respuesta = await fetch(`${API_BASE_URL}/api/guias-inspeccion/1/secciones/E`);
        if (!respuesta.ok) {
        ...
        setGrupos(agruparPorArticulo(datosApi.items));
              ...
              console.error('Error al cargar la Sección E:', err);
              setError('No se pudo cargar la Sección E. Verificá que el backend esté corriendo.');
       } finally {
        setCargando(false);
      }
    }
  ...
  return (
    <div className="pagina">
      ...
      <nav className="tabs">
        {TABS.map((tab, i) => (
          <span key={tab} className={`tabs__item ${i === 6 ? 'tabs__item--activo' : ''}`}>
            {tab}
          </span>
        ))}
      </nav>
      ...
      <main className="tarjeta">
        <div className="tarjeta__encabezado">
          <span className="tarjeta__etiqueta">SECCIÓN E</span>
          <div className="tarjeta__titulo-fila">
            <h2>Salud e Higiene del Personal</h2>
...
export default FormularioSeccionE;